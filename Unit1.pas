...
var
 F12: Integer;
begin
 Warning:='Вы уверены что хотите списать оборудование?';
 Delete_help:='Списание_оборудование';
...
begin
 ...
 //frx_Memo.Text:= 'Барабан № ' + IntToStr(DM.MTable_BARABAN.FieldByName('ID').AsInteger);
...
var
 F12: Integer;
begin
 Delete_help:='Удаление_поверка';
 Warning:='Вы уверены что хотите удалить запись о поверки';
 F12:=Form12.ShowModal;
end;
...
var
 ...
 FIO, F, I, O: string;
 len_type: real;  // количество символов в типе оборудования
 len_mf_num: real; // количество символов в заводском номере
...
begin
 ...
 frx_Memo1.Text:= 'Тип: ' + DM.MTable_SI_IO.FieldByName('TYPE_SI_IO').AsString;


 frx_Memo2:=Form1.frxrprt2.FindObject('Memo2')as TfrxMemoView;
 frx_Memo2.Text:='Зав. №: ' + DM.MTable_SI_IO.FieldByName('N_ZAVOD').AsString;

 // frx_Memo3:=Form1.frxrprt2.FindObject('Memo3')as TfrxMemoView;
 // frx_Memo3.Text:='Ответственный: ' + FIO;
...
begin
 ...
 begin
   MessageDlg('Скан не подгружен', mtInformation, [mbOk], 0)
 end
 else
 begin
...
begin
 ...
 frx_Memo2.Text:='Последней: ' +  DateToStr(DM.MTable_SI_IO.FieldByName('DATE_POVERKA_LAST').AsDateTime);

 frx_Memo3:=Form1.frxrprt3.FindObject('Memo3')as TfrxMemoView;
 frx_Memo3.Text:='Очередной: ' + DateToStr(DM.MTable_SI_IO.FieldByName('DATE_POVERKA_NEXT').AsDateTime);
...
begin
 ...
 begin
 Check_Nas_Path_Var:=Copy(DM.DSet_Pablic.FieldByName('PATH').AsString, 1, 5); // проверка, есть ли в пути nas//(1-5 символов)
